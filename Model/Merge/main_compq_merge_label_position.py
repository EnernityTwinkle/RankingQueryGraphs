    ...
    if args.do_train:   
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                loss = (loss_point + loss_pair + loss_list)
                # loss.backward()   
                loss_point.backward(retain_graph=True)
                loss_pair.backward(retain_graph=True)
                loss_list.backward()   
    ...
    for seed in [42]:
        for steps in[100]:
            ...
            for N in [30]:
                ...
                # client = WatchClient(id="webq_mergexx2" + str(N) + str(seed) + str(steps), gpus=[1],
                #                     server_host="127.0.0.1", server_port=62333)
                # client.wait()
                print(seed)
                os.environ["CUDA_VISIBLE_DEVICES"] = '2'
                parser = ArgumentParser(description = 'For KBQA')
                parser.add_argument("--data_dir",default='../../../bert_rank_data/compq/',type=str)
                ...
                parser.add_argument("--output_dir",default='../../../bert_rank_data/model/compq/bert_compq_3backward_merge_type_entity_time_ordianl_mainpath_neg_' + str(N) + '_' + str(seed) + '_' + str(steps) + '/',type=str)
                ...
                parser.add_argument("--T_file_name",default='compq_rank1_f01_label_position_listwise_1_' + str(N) + '_type_entity_time_ordinal_mainpath_is_train.txt',type=str)
                parser.add_argument("--v_file_name",default='compq_rank1_f01_label_position_listwise_1_70_type_entity_time_ordinal_mainpath_dev_all.txt',type=str)
                parser.add_argument("--t_file_name",default='compq_rank1_f01_label_position_listwise_1_70_type_entity_time_ordinal_mainpath_test_all.txt',type=str)
